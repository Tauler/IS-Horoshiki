...
BEGIN
	...
	-- считает норма часы только один раз на дату
	UPDATE
			#RESULT_USER_SHIFT
	   SET
			[NormaHour] = [dbo].[GetScheduleShiftPersonalNormaHour] (UserId, 0, [DateDoc], [DateDoc])
			...
			[RowID] >   (
							SELECT
							 ...
							 FROM
									#RESULT_USER_SHIFT b
						    WHERE 
									#RESULT_USER_SHIFT.DepartmentId		= b.DepartmentId	AND
									(
										(#RESULT_USER_SHIFT.SubDepartmentId IS NOT NULL AND #RESULT_USER_SHIFT.SubDepartmentId	= b.SubDepartmentId) OR
										(#RESULT_USER_SHIFT.SubDepartmentId IS NULL AND b.SubDepartmentId IS NULL) 
									) AND
									#RESULT_USER_SHIFT.UserId			= b.UserId			AND
									#RESULT_USER_SHIFT.ShiftPersonalScheduleDate = b.ShiftPersonalScheduleDate
						)
